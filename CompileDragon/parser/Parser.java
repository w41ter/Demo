...
public class Parser {
	...
	Exper equality() throws IOException {
		Exper x = rel();
	...
	Exper rel() throws IOException {
		Exper x = exper();
		switch (look.tag) {
			case '<': case Tag.LE: case Tag.GE: case '>':
				Token tok = look; move(); return new Rel(tok, x, exper());
			default:
				return x;
		}
	}
	Exper exper() throws IOException {
		Exper x = term();
	...
	Exper unary() throws IOException {
		if (look.tag == '-') {
	...
	Exper factor() throws IOException {
		Exper x = null;
		switch (look.tag) {
			case '(':
				move(); x = bool(); match(')'); return x;
			case Tag.NUM:
